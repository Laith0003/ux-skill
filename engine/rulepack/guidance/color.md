...
Color sets every surface, text, line, fill and ring in the product, in light and dark and at standard and high contrast. It is generated from one brand color and the axes, and the brand color is the identity: it leads. The brand's role is that it fills the main action (fill), marks words and links while the main action is ink (accent), or draws edges, rules and underlines while actions and links are ink (edge). A saturated brand that stands apart from the page and from ink fills the action whatever the brief, yellows and navies included; accent or edge wins only where the brand is near grey, a near white tint or a near black, or a formal brief meets a brand that cannot carry a fill (decisions/brand-leads-by-reach.md). The brand anchors its ramp at step 500. A supporting accent takes a second hue: a warm brand's is placed by contrast and warmth, a cool brand's moves with warmth, formality and contrast inside its own family, and neither ever pairs a blue with a pink or a purple (decisions/support-in-the-brand-family.md); a grey brand's is a neutral step, since an identity with no hue gains none (decisions/grey-support-is-neutral.md). The neutral ramp takes its temperature from the brand, and the warmth axis leans it along the brand's own hue (decisions/neutrals-lean-along-the-brand.md); a grey brand steers no hue (decisions/grey-brands-steer-no-hue.md), and four status ramps lean toward the brand inside a fixed band of their own hues (decisions/status-harmony.md). The build measures text roles against every surface they can sit on, the input line, the selected line, the error edge, the accent line and the focus ring against every surface a control sits on (the page, card, sunken and raised surfaces, the tint, the band and the table stripe), and the text on each fill against that fill. A control on the brand band takes color.text.on-brand for its focus ring and its edges (decisions/controls-on-brand-surfaces.md), and the primary button there takes its own fill, color.action.on-brand. The primary button keeps the exact brand color as its fill whenever its text reads naturally on it; black text on a saturated mid tone weighs against the least darkening of the brand that carries white text, one cost in light and dark, weighing the focus ring in dark high contrast (decisions/natural-fill-for-white-text.md). A control's fill, or the edge around it, is measured against every surface its contract places it on, in every scheme: the primary edge, the danger fill and the danger edge against every surface a control sits on, and the button on the brand band against the band (decisions/fills-on-every-control-surface.md). Links are measured on every status soft fill (decisions/links-on-status-soft-fills.md). The other strong status fills are measured against the page. The focus ring is also measured against the primary fill, and under high contrast never measures less than the standard ring (decisions/ring-never-weaker.md). Contracts add the pairings their components need. Color does not govern type size, spacing or shadow; those foundations own them.
...
- `color.action.primary`: the fill of the one main action on a view. What it is depends on the brand's role, which the system report states. With fill it is the brand: the exact brand color whenever its text reads naturally on it, else the least darkening of the brand that carries white text (color.brand.fill), or the nearest brand step whose text reads. With accent or edge it is ink, a neutral from the far end of the ramp, and the brand is not on the button. The handoff for a built system states which.
- `color.action.primary-edge`: the edge of the primary button: the fill itself when the fill clears every surface the button sits on (page, card, sunken, raised, tint, band, stripe and header), else the nearest step of its ramp that does.
...
- `color.action.on-brand-hover`: the button on the brand band under a pointer.
- `color.action.on-brand-pressed`: the button on the brand band while pressed.
...
| The one action on the brand band | the primary button with surface set to brand: color.action.on-brand with color.text.on-brand-action | the default primary, whose fill can match the band |
...
| A closing call to action in the brand color | color.surface.brand with color.text.on-brand, and its button with surface set to brand | a primary button stretched wide |
...
- A filled control placed on a surface its contract does not name is measured there before it ships; the contract check measures every action fill, or its edge, on every surface the contract names (decisions/fills-on-every-control-surface.md).
