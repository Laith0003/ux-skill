...
Color sets every surface, text, line, fill and ring in the product, in light and dark and at standard and high contrast. It is generated from one brand color and the axes, and the brand color is the identity: it leads. The brand's role is that it fills the main action (fill), marks words and links while the main action is ink (accent), or draws edges, rules and underlines while actions and links are ink (edge). A saturated, mid-tone brand fills the action whatever the brief; accent or edge wins only where the brand is very light, very dark or near grey, or a formal brief meets a brand that cannot carry a fill (decisions/brand-leads-the-role.md). The brand anchors its ramp at step 500. A supporting accent takes a second hue: a cool brand's leans to its own hue at low chroma, so no blue sits beside a pink or a purple, and a grey brand's runs from violet to rose, clear of every status hue (decisions/support-clear-of-banned-pairs.md, decisions/grey-accent-clear-of-status.md). The neutral ramp takes its temperature from the brand first, and the warmth axis only leans it (decisions/neutrals-follow-the-brand.md); a grey brand steers no hue (decisions/grey-brands-steer-no-hue.md), and four status ramps lean toward the brand inside a fixed band of their own hues (decisions/status-harmony.md). The build measures text roles against every surface they can sit on, the input line, the selected line, the error edge, the accent line and the focus ring against every surface a control sits on (the page, card, sunken and raised surfaces, the tint, the band and the table stripe), and the text on each fill against that fill. A control on the brand band takes color.text.on-brand for its focus ring and its edges (decisions/controls-on-brand-surfaces.md), and the primary button there takes its own fill, color.action.on-brand. The primary button keeps the exact brand color as its fill whenever its text reads naturally on it; black text on a saturated mid tone weighs against a move to the nearest step that carries white text, weighing the focus ring in dark high contrast (decisions/natural-text-on-the-brand.md). A control's fill, or the edge around it, is measured against every surface its contract places it on, in every scheme: the primary edge and the danger fill against the page, card, sunken and raised surfaces, and the button on the brand band against the band (decisions/fills-on-every-placement.md). The strong status fills are measured against the page. The focus ring is also measured against the primary fill, and under high contrast never measures less than the standard ring (decisions/ring-never-weaker.md). Contracts add the pairings their components need. Color does not govern type size, spacing or shadow; those foundations own them.
...
- `color.action.primary`: the fill of the one main action on a view. What it is depends on the brand's role, which the system report states. With fill it is the brand: the exact brand color whenever its text reads naturally on it, else the nearest brand step whose text does, often a step darker with white text. With accent or edge it is ink, a neutral from the far end of the ramp, and the brand is not on the button.
- `color.action.primary-edge`: the edge of the primary button: the fill itself when the fill clears every surface the button sits on (page, card, sunken and raised), else the nearest step of its ramp that does.
...
Some roles carry no contrast minimum. Disabled text and fills are checked for distinctness instead, since WCAG exempts inactive controls; the button contract still holds its disabled label to our 1.3:1 floor on the disabled fill (decisions/disabled-contrast.md). The subtle separator is decorative and is checked for distinctness from the surfaces it divides. The scrim is translucent and never enters a pairing; content sits on the raised surface above it. A hover or pressed fill is measured with the text on it, never as an overlay. The primary edge and the danger fill with its hover and pressed steps are measured against every surface the button sits on, and the button on the brand band against the band; the strong status fills are measured against the page (decisions/fills-on-every-placement.md).

## Checks

- `on-color-natural`: black text on the primary fill where a brand step nearer to the brand than the fill plus what black text costs there (a saturated mid tone) carries white text; the fill should be that step with white text, our rule.
...
- A filled control placed on a surface its contract does not name is measured there before it ships; the contract check measures every action fill, or its edge, on every surface the contract names (decisions/fills-on-every-placement.md).
