...
- `type.phone.figure`: the factor type.text.figure takes on a phone: heading-1's, since the two share a step, so a figure never outranks the headline there.
{arabic} - `type.fit.<style>.<tier>`: the factor type.text.<style>'s size and letter spacing take from the <tier> breakpoint up: at most 1, and small enough that a 13 letter Latin word and a 10 letter Arabic word at that size fit the tier's column (all of it on a tablet, seven twelfths from the laptop up), measured with the face's own average advance, while the four largest styles keep falling in size; tokens.css applies it (decisions/landing-display-step.md). The figure takes heading-1's.
{latin} - `type.fit.<style>.<tier>`: the factor type.text.<style>'s size and letter spacing take from the <tier> breakpoint up: at most 1, and small enough that a 13 letter word at that size fits the tier's column (all of it on a tablet, seven twelfths from the laptop up), measured with the face's own average advance, while the four largest styles keep falling in size; tokens.css applies it (decisions/landing-display-step.md). The figure takes heading-1's.
...
- `type.text.label`: an eyebrow above a heading, a tag or a line of metadata, spaced open; set in the mono face in a technical system. An eyebrow is text only: never a line, dash or dot before or after it (decisions/eyebrow-is-text.md). When the project has an existing design system, its own label style wins: a system that sets its labels at zero tracking or in sentence case keeps them that way (decisions/existing-system-wins.md).
...
{arabic} - `phone-hierarchy`: on a phone display, hero, heading-1 and section-title keep falling in size above heading-2, in both directions, each factor sits above 0 and at most 1, and the figure never sits above heading-1.
{latin} - `phone-hierarchy`: on a phone display, hero, heading-1 and section-title keep falling in size above heading-2, each factor sits above 0 and at most 1, and the figure never sits above heading-1.
{arabic} - `display-fits`: from the tablet up, a 13 letter Latin word and a 10 letter Arabic word at the display size fit each tier's column, and the display, hero, heading-1 and section-title keep falling, 1.08 times apart, above heading-2.
{latin} - `display-fits`: from the tablet up, a 13 letter word at the display size fits each tier's column, and the display, hero, heading-1 and section-title keep falling, 1.08 times apart, above heading-2.
