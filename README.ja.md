...
> **AI コーディング向け最強の UX プラグイン。** 11 個のクエリ可能な JSON マニフェスト(84 のスタイル、176 のパレット、70 のタイプペアリング、148 のコンポーネント、184 の業種、35 のチャートタイプ、57 のモーションプリセット、112 の UX ロー、145 のアンチパターンルール、25 の技術スタック、160 のブランド仕様)、22 のスラッシュコマンド、5 つのサブエージェント、そして決定論的な反 AI スロップリンターを備えた Python 推論コア。クロス IDE 対応:Claude Code、Cursor、Windsurf、GitHub Copilot、Gemini CLI、Codex、Kiro、Cline、Continue、Aider、Zed、JetBrains AI、Pieces、Tabby、Tabnine、CodeWhisperer、Roo Cline に同梱されます。
...
このプラグインはあなたと AI コーディングツールの間に座ります。Claude Code、Cursor、その他の AI アシスタントに「フィンテックのランディングページを作って」と頼むと、アシスタントは通常即興で作ります——そして結果は 5 秒で AI 生成と見抜かれます(紫から青のグラデーション、等大の 3 枚カード、ディスプレイサイズの Inter、推薦文の「John Doe」、デフォルト 300ms のトランジション、中央寄せのヒーロー、CTA で跳ねる矢印)。ux-skill は即興を**構造化された制約**で置き換えます:`/ux-discover` でブリーフを取り、`/ux-recommend` でシステムを選び、`/ux-design` でコードを生成し、`/ux-lint` でコミット前に 145 個の決定論的な反 AI スロップルールを通すことを検証します。
...
1. [クイックインストール](#クイックインストール)
2. [数字 — トップ 8 の Claude UX スキルとのライブ比較](#数字--トップ-8-の-claude-ux-スキルとのライブ比較)
3. [アーキテクチャ — 各部品がどう噛み合うか](#アーキテクチャ--各部品がどう噛み合うか)
4. [22 のスラッシュコマンド — 詳細リファレンス](#22-のスラッシュコマンド--詳細リファレンス)
5. [5 つのサブエージェント](#5-つのサブエージェント)
6. [11 のデータマニフェスト](#11-のデータマニフェスト)
7. [120 の反 AI スロップルール — リンター](#120-の反-ai-スロップルール--リンター)
8. [160 のブランド DESIGN.md 仕様 — カテゴリ別](#160-のブランド-designmd-仕様--カテゴリ別)
9. [MCP サーバー — 非対称な一手](#mcp-サーバー--非対称な一手)
10. [17 IDE 向けインストーラー](#17-ide-向けインストーラー)
11. [ユースケース — 具体的なシナリオ](#ユースケース--具体的なシナリオ)
12. [他の選択肢との比較](#他の選択肢との比較)
13. [ロードマップ](#ロードマップ)
14. [コントリビューション](#コントリビューション)
15. [ライセンス、作者、謝辞](#ライセンス作者謝辞)
...
これで 22 のスラッシュコマンドと 5 つのサブエージェントが Claude Code セッションに接続されます。インストール後、`/ux-init` を実行してプロジェクト単位の `.ux/` 状態ディレクトリをセットアップし、Python エンジンが到達可能であることを検証してください。
...
| **Laith0003/ux-skill (ux-skill)** | **14** | **Python エンジン + 11 マニフェスト + 22 コマンド + 5 サブエージェント + CI リンター** | **22** | **145 の正規表現ルール** | **160** | **148** | **57** | **17** |
...
│
├── commands/                          22 の Claude Code スラッシュコマンド (.md)
│   ├── ux-init.md                     ブートストラップ
...
- **何をするか:** バージョン + 11 データマニフェストのエントリ件数を表示し、何がインストールされているか検証できるようにします。
...
- **何をするか:** Python エンジンの 5 並列検索を 11 マニフェスト横断で実行し、マージされたデザインシステムを 1 つ返します。業種 → スタイル → パレット → タイプ → モーション + コンポーネント + ブランド事例 + ガードレール。
...
完全なルール一覧は [120 の反 AI スロップルール](#120-の反-ai-スロップルール--リンター)。
...
|---|---|---|
| Claude Code | `.claude/` または `CLAUDE.md` | `.claude-plugin/plugin.json` のプラグインマニフェスト + 全 22 コマンド + 全 5 サブエージェント |
...
**ux-skill** · **v3.0.0-stable** · Claude Code、Cursor、Windsurf、その他すべての AI コーディングツールが、AI 生成と読まれないフロントエンドを出力するために構築。
