...
| `outline-none-no-focus-visible` | Per rule block. A removed outline passes only when a focus rule covers the whole removal selector, matched compound by compound from the right, and draws a visible ring (outline, box-shadow or border that is not `none`, `0` or transparent), or when a `:focus-within` or `:has(...:focus-visible)` rule sits on an ancestor of that same element. A ring in `@media print`, on a hover state, or on another element never counts. For inline styles only the element's own classes count, and `focus:outline-none` or `ring-0` is not a ring. |
...
| `imagery-mandatory-missing` | Skips a page only when one article, form, table, code listing or grid holds most of the visible text of `<main>` (or `<body>`), or forms, tables, listings and grids together do. Section counts and the mere presence of a form decide nothing, so a landing page built from divs, a one-section hero with a waitlist form, and a testimonials page with a pricing table still need imagery. |
