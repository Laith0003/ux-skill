...
| `product_type` | `app`, `software`, `marketing-site`, `editorial`, `commerce` | "a pharmacy ordering app" gives `app`; "a landing page for the company" gives `marketing-site`; "an online shop" gives `commerce`; "a magazine" or "a journal" gives `editorial`. An app or software leans the display face to sans and the Arabic face to an interface face; an editorial product may take a serif, the more so the more humanist and formal the brief |
...
- That the landing display, hero, heading-1 and section-title step down on phones on their own; the page needs no media query for them. A landing page's headline takes `type.text.display`, its proof numbers `type.text.figure` and the gap between its sections the landing gap alias, `layout.landing-gap`, which tokens.css switches per viewport.
