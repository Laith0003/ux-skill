...
- [`anthropic`](anthropic-DESIGN.md) — AI safety company corporate brand. Putty canvas, Tiempos serif at every tier, red-clay link accent, narrow editorial column.
- [`apple`](apple.md) — Consumer electronics. Premium white space, SF Pro, cinematic imagery.
- [`apple-music`](apple-music-DESIGN.md) — Streaming music. White canvas, Apple Music red CTAs, album-cover-driven hero, floating circular play-button.
...
- [`clay`](clay.md) — Creative agency. Organic shapes, soft gradients, art-directed layout.
- [`clickhouse`](clickhouse.md) — Fast analytics database. Yellow-accented, technical documentation style.
- [`cohere`](cohere.md) — Enterprise AI platform. Vibrant gradients, data-rich dashboard aesthetic.
...
- [`dbt`](dbt-DESIGN.md) — Analytics engineering platform. Dual-canvas cream-and-navy-violet, dbt-orange accent, isometric pixel-cube glyph, SQL editor hero.
- [`dezeen`](dezeen-DESIGN.md) — Architecture-and-design magazine. Strict monochrome chrome, custom serif display, 4-column masonry photo grid, image hover-zoom.
- [`economist`](economist-DESIGN.md) — British weekly newspaper of record. Red 1.5cm masthead bar, Milo Serif slab, no bylines, drop caps on features.
...
- [`fivetran`](fivetran-DESIGN.md) — Managed data pipelines. Cobalt-blue CTAs on white, Tiempos serif moments, 200+ connector-logo grid as hero device.
- [`fly-io`](fly-io-DESIGN.md) — Edge compute platform. Warm near-black canvas, hot-pink-to-violet gradient mark, Lora serif body, real shell snippets as content.
...
- [`instagram`](instagram-DESIGN.md) — Photo-and-video social platform. Monochrome chrome with multi-stop yellow-magenta-violet gradient on brand mark only, 3-column square grid, circular avatars with gradient unread-ring.
- [`intercom`](intercom.md) — Customer messaging. Friendly blue palette, conversational UI patterns.
- [`kraken`](kraken.md) — Crypto trading. Purple-accented dark UI, data-dense dashboards.
- [`lamborghini`](lamborghini.md) — Supercar brand. True black surfaces, gold accents, dramatic uppercase typography.
...
- [`modal`](modal-DESIGN.md) — Serverless Python for AI. White canvas with exposed-grid dividers, mustard accent, live Python code-block as hero, square geometry.
...
- [`n26`](n26-DESIGN.md) — European premium neobank. White canvas, Tiempos serif hero, cyan-mint CTAs, floating credit card on pastel gradient.
...
- [`openai`](openai-DESIGN.md) — AI platform of record. Pure black hero band over pure white body, Söhne at every tier, photography hero on black, zero decorative chrome.
...
- [`perplexity`](perplexity-DESIGN.md) — Answer engine. Graphite app canvas + cream marketing, peacock-teal accent, FK Grotesk Neue, search-pill IS the hero, source-favicon citation ribbons.
...
- [`railway`](railway-DESIGN.md) — Infrastructure deployment. Midnight-violet canvas, lavender CTA voltage with glow, Berkeley Mono everywhere, service-graph hero with pulsing nodes.
...
- [`retool`](retool-DESIGN.md) — Internal-tool builder. Cream canvas, GT Sectra serif at magazine-cover scale, signal-orange CTA, dashboard-mockup hero + operator logo wall.
- [`revolut`](revolut.md) — Digital banking. Sleek dark interface, gradient cards, fintech precision.
- [`robinhood`](robinhood-DESIGN.md) — Retail investing platform. Warm near-black canvas, Robinhood Green voltage, tabular-figure mono numerals, portfolio chart hero with green/red line.
- [`runwayml`](runwayml.md) — AI video generation. Cinematic dark UI, media-rich layout.
- [`sanity`](sanity.md) — Headless CMS. Red accent, content-first editorial layout.
...
- [`snowflake`](snowflake-DESIGN.md) — Data cloud platform. Glacier-blue gradient hero bands, Tiempos Headline serif at hero scale, hexagon snowflake-lattice motif as background.
...
- [`theverge`](theverge.md) — Tech editorial media. Acid-mint and ultraviolet accents, Manuka display, rave-flyer story tiles.
- [`tiktok`](tiktok-DESIGN.md) — Short-form vertical video. Pure black canvas, cyan-magenta chromatic-aberration brand mark, 9:16 video feed at full viewport, right-rail floating icon stack.
- [`together.ai`](together.ai.md) — Open-source AI infrastructure. Technical, blueprint-style design.
- [`uber`](uber.md) — Mobility platform. Bold black and white, tight type, urban energy.
- [`vercel`](vercel.md) — Frontend deployment. Black and white precision, Geist font.
- [`vodafone`](vodafone.md) — Global telecom brand. Monumental uppercase display, Vodafone Red chapter bands.
- [`voltagent`](voltagent.md) — AI agent framework. Void-black canvas, emerald accent, terminal-native.
- [`warp`](warp.md) — Modern terminal. Dark IDE-like interface, block-based command UI.
- [`webflow`](webflow.md) — Visual web builder. Blue-accented, polished marketing site aesthetic.
- [`wired`](wired.md) — Tech magazine. Paper-white broadsheet density, custom serif display, mono kickers, ink-blue links.
- [`wise`](wise.md) — Money transfer. Bright green accent, friendly and clear.
- [`wsj`](wsj-DESIGN.md) — American financial newspaper of record. Black-on-white broadsheet, Escrow display serif, hedcut dot-stippled portraits, market-data ticker masthead.
- [`x.ai`](x.ai.md) — Elon Musk's AI lab. Stark monochrome, futuristic minimalism.
- [`zapier`](zapier.md) — Automation platform. Warm orange, friendly illustration-driven.

## How to use

1. **User names a brand**: the user's brief includes "make it look like X" or "match the Y aesthetic" or pastes a brand reference.
2. **Match against this index**: find the closest brand in the list above. Substring match is fine ("stripe" → `stripe.md`).
3. **Read the full brand file**: pull the complete DESIGN.md spec for that brand.
4. **Pass to frontend-engineer**: include the brand spec verbatim in the sub-agent's prompt. The brand becomes the visual ground truth; the plugin's anti-slop discipline applies on top (no AI fingerprints, full SEO surface, etc.).
5. **Echo back**: confirm to the user which brand the output is matching.

## When the user names a brand NOT in the library

Offer to:
- **Adapt the closest match** — e.g., user wants "Mailchimp aesthetic" → propose adapting Intercom (closest in spirit) or Notion (closest in warmth).
- **Run discovery on the brand** — ask the user for 3-5 references of that brand's surfaces and build a custom brand spec inline.
- **Use the closest category default** — pick from a generic style library (`references/styles/library.md`).
...
To refresh a brand's spec to the latest version:
```
npx getdesign@latest add <brand>
```
Then commit the updated `references/brands/<brand>.md`.

To add a new brand not in this catalog, propose a PR with a complete DESIGN.md spec following the format used in the existing brand files.
