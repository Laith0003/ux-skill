...
Three distinct aesthetic systems for use as design directions. Each has its own typography, color, layout, motion, and component logic. When picking a style for a brief, choose ONE and commit; don't blend.
...
Typography carries almost all visual weight. There are two compulsory voices: a structural heavy sans for headlines and a monospace for metadata. Optionally, a third high-contrast serif appears as textural disruption.
...
- Use technical monospaces. Suitable choices include modern programming monospaces, plex-style technical monos, bitmap-era terminal faces, or typewriter monospaces.
...
Textural disruption (used sparingly, often once per page):
...
The color architecture is uncompromising. Modern translucency, soft shadows, and full-spectrum gradients are banned. Pick ONE substrate per project — light industrial print or dark tactical telemetry — and stay there.
...
- Accent: a single aviation or hazard red, `#E61919` or `#FF2A2A`. This is the only accent color. It appears as strike-throughs, thick structural dividers, warning stripes, or vital data highlights. Never decorative.
...
- Foreground: white phosphor, `#EAEAEA`. This is the primary text color.
- Accent: the same hazard red, used under the same rules.
- An optional terminal green (`#4AF626`) may appear, but only on a single specific element such as one status indicator or one live readout. Never as a general body color. If it doesn't serve a clear purpose, omit it entirely.
...
The signature elements that announce this style:
...
- Inter, Roboto, Open Sans, Helvetica, or any default-feeling humanist sans.
...
Short descriptive language that captures the feeling without naming brands:
...
The system relies on extreme typographic contrast and deliberate font selection to establish an editorial feel. No standard SaaS defaults.

Primary sans-serif (body, UI, buttons, navigation):

- Use clean, geometric, character-bearing system or near-system sans-serifs. Suitable choices include modern display-and-text sans-serifs designed for screen, neo-grotesque humanist hybrids, or system-native premium faces.
...
- Use high-quality contemporary text serifs or display serifs with character. Suitable choices include modern transitional serifs with subtle quirks, contemporary "magazine" serifs, or display serifs with optical contrast.
- Tracking is tight: `-0.02em` to `-0.04em`.
- Line-height is compressed: `1.05` to `1.15` at large sizes.
- The serif is used surgically. One per hero, maybe one per major section. Not as a body face.

Monospace (code, keystrokes, metadata, small status text):

- Use modern, well-designed programming monospaces with humanist proportions rather than terminal-era faces.
- Always one size step smaller than surrounding body text.

Text colors:

- Body never uses absolute black (`#000000`). Off-black or charcoal — `#111111` or a warm charcoal around `#2F3437` — paired with generous line-height for legibility.
...
Signature elements that announce this style:
...
- Use modern variable display sans-serifs with wide proportions, or contemporary display serifs with high optical contrast. Suitable choices include wide geometric grotesks, expressive editorial serifs with personality, or modern neo-grotesques with optical sizes.
...
- Use tabular figures where columns of numbers appear together so digits align vertically.
...
| AI product targeting broad audience | Luxe and ambitious | Wants to feel a leap | High-end |
...
When the brief says "wow, agency, awwwards, design-led" without further specifics, default to high-end. The brief is signaling that surface ambition is the explicit requirement.
...
- Avoid motion parallax that depends on device orientation sensors. Cursor-driven parallax is acceptable; phone-tilt parallax is not, because it requires sensor permissions that erode trust.
