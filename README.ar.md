...
7. [قواعد مكافحة ركاكة الذكاء الاصطناعي الـ 100 — المُدقّق](#قواعد-مكافحة-ركاكة-الذكاء-الاصطناعي-الـ-100--المُدقّق)
8. [مواصفات DESIGN.md الـ 110 للعلامات — حسب الفئة](#مواصفات-designmd-الـ-110-للعلامات--حسب-الفئة)
9. [خادم MCP — التحرّك غير المتماثل](#خادم-mcp--التحرّك-غير-المتماثل)
10. [مثبّت 17 بيئة تطوير](#مثبّت-17-بيئة-تطوير)
11. [حالات استخدام — سيناريوهات ملموسة](#حالات-استخدام--سيناريوهات-ملموسة)
12. [المقارنة مع البدائل](#المقارنة-مع-البدائل)
13. [خارطة الطريق](#خارطة-الطريق)
14. [المساهمة](#المساهمة)
15. [الترخيص، المؤلّف، الشكر](#الترخيص-المؤلّف-الشكر)
...
| **Laith0003/ux-skill (ux-skill)** | **14** | **محرّك Python + 11 ملفّ بيانات + 22 أمرًا + 5 وكلاء + مُدقّق CI** | **22** | **100 قاعدة regex** | **110** | **148** | **57** | **17** |
...
- **مواصفات العلامات:** 110 ملفّات DESIGN.md حقيقية (Apple، Stripe، Linear، Figma، Tesla، BMW، Notion، Spotify، Airbnb، Vercel، Supabase، Cursor، Raycast، Claude، و96 علامة أخرى). لا تُصدر إضافة منها مكتبة علامات.
...
4. **الإخراج.** ملفّ JSON يحتوي الأسلوب المُختار، ولوحة الألوان المُختارة، والاقتران الطباعي، وأفضل 5 مهيّآت حركة، وأفضل 12 مكوّنًا، وأفضل 5 علامات نموذجيّة، وكلّ حواجز الأنماط الضارّة الـ 100 وهي مُفعّلة. مع كتلة تبرير تشرح كلّ خيار.
...
6. **التحقّق.** يُعيد `/ux-lint` فحص الكود المُولَّد ضدّ قواعد regex الـ 100. يخرج بشيفرة عدم نجاح في حال وُجدت ملاحظات Critical/High داخل CI.
...
- **الاستدعاء (في Claude Code):**
  ...
  **الاستدعاء (في سطر الأوامر):**
  ...
  ux recommend \
    ...
    --region=mena
  ```
- **الإخراج:** يكتب `.ux/last-recommendation.json` — الأسلوب المختار، لوحة الألوان المختارة، الاقتران الطباعي، أفضل 5 مهيّآت حركة، أفضل 12 مكوّنًا، أفضل 5 علامات نموذجيّة، حواجز الأنماط الضارّة الـ 100 وهي مُفعَّلة، إلى جانب التبرير.
...
3. حواجز الأنماط الضارّة الـ 100 (مُفعّلة دومًا).
...
قائمة القواعد الكاملة في [قواعد مكافحة ركاكة الذكاء الاصطناعي الـ 100](#قواعد-مكافحة-ركاكة-الذكاء-الاصطناعي-الـ-100--المُدقّق).

### `brands/*.json` — 110 مواصفة علامة

| الحقل | الوصف |
|---|---|
| `entries` | 110 (إضافة إلى `_index.json` الذي يسرد الجميع) |
...
القائمة الكاملة في [مواصفات DESIGN.md الـ 110 للعلامات](#مواصفات-designmd-الـ-110-للعلامات--حسب-الفئة).

---

## قواعد مكافحة ركاكة الذكاء الاصطناعي الـ 100 — المُدقّق
...
## مواصفات DESIGN.md الـ 110 للعلامات — حسب الفئة
...
| مواصفات العلامات | **110** | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 0 |
| قواعد الأنماط الضارّة | **100** | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 0 |
