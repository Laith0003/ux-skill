...
> **AI coding için en güçlü UX plugin'i.** 11 sorgulanabilir JSON manifesti olan Python akıl yürütme çekirdeği (84 stil, 176 palet, 70 tipografik eşleşme, 148 component, 184 sektör, 35 chart tipi, 57 motion preset, 112 UX yasası, 100 anti-pattern kuralı, 25 tech stack, 110 brand spec), 22 slash komutu, 5 sub-agent ve deterministik anti-AI-slop linter. IDE'ler arası: Claude Code, Cursor, Windsurf, GitHub Copilot, Gemini CLI, Codex, Kiro, Cline, Continue, Aider, Zed, JetBrains AI, Pieces, Tabby, Tabnine, CodeWhisperer ve Roo Cline'a dağıtılır.
...
Plugin, sen ve AI coding aracı arasında durur. Claude Code'a, Cursor'a veya başka bir AI asistanına «bir fintech landing oluştur» dediğinde, asistan tipik olarak doğaçlama yapar — ve sonuç beş saniye içinde AI üretimi olarak okunur (mor-mavi gradyanlar, üç eşit card, display boyutunda Inter, testimonial'larda «John Doe», 300ms varsayılan geçişler, ortalanmış hero, zıplayan ok CTA'lar). ux-skill doğaçlamayı **yapılandırılmış kısıtlamalarla** değiştirir: brief'i yakalamak için `/ux-discover`, sistemi seçmek için `/ux-recommend`, kodu üretmek için `/ux-design` ve commit'ten önce 100 deterministik anti-AI-slop kuralından geçtiğini doğrulamak için `/ux-lint` çalıştırırsın.
...
8. [110 brand DESIGN.md spec'i — kategoriye göre](#110-brand-designmd-speci--kategoriye-göre)
...
| **Laith0003/ux-skill (ux-skill)** | **14** | **Python motoru + 11 manifest + 22 komut + 5 sub-agent + CI linter** | **22** | **100 regex kuralı** | **110** | **148** | **57** | **17** |
...
- **Anti-pattern linter:** 100 deterministik regex kuralı, CI'da çalışır, Critical/High'ta non-zero ile çıkar. Diğerlerinin hiçbiri deterministik linter dağıtmıyor.
- **Brand spec'leri:** 110 gerçek DESIGN.md spec (Apple, Stripe, Linear, Figma, Tesla, BMW, Notion, Spotify, Airbnb, Vercel, Supabase, Cursor, Raycast, Claude ve 96 daha). Diğerlerinin hiçbiri brand kütüphanesi dağıtmıyor.
...
│   └── brands/*.json                  110 brand DESIGN spec'i + _index.json
│
...
4. **Çıktı.** Seçilen stil, palet, tipografik çift, top 5 motion preset'i, top 12 component, top 5 brand örneği ve aktif 100 anti-pattern guardrail'ının hepsiyle bir JSON dokümanı. Artı her seçimi açıklayan bir rationale bloğu.
...
6. **Doğrulama.** `/ux-lint` üretilen kodu 100 regex kuralına karşı yeniden tarar. CI'da Critical/High'ta non-zero ile çıkar.
...
- **Çağırma (Claude Code):**
  ...
  **Çağırma (CLI):**
  ...
  ux recommend \
    ...
    --region=mena
  ```
- **Çıktı:** `.ux/last-recommendation.json` yazar — seçilen stil, seçilen palet, seçilen tipografik çift, top 5 motion preset'i, top 12 component, top 5 brand örneği, aktif 100 anti-pattern guardrail'ı, artı rationale.
...
- **Ne yapar:** Kodun üzerinde 100 regex kuralı çalıştırır. LLM çağrısı yok. CI'da Critical / High'ta non-zero ile çıkar. Kaynak: `data/anti-patterns.json`. Kurallar A11y (23), Content (15), Layout (13), Typography (10), Color (9), Quality (9), Visual (9), Motion (8), Performance (4) kapsar.
...
3. 100 anti-pattern guardrail'ı (her zaman aktif).
...
### `anti-patterns.json` — 100 regex kuralı
...
### `brands/*.json` — 110 brand spec'i

| Alan | Açıklama |
|---|---|
| `entries` | 110 (artı tümünü listeleyen `_index.json`) |
...
Tam liste [110 brand DESIGN.md spec'i](#110-brand-designmd-speci--kategoriye-göre)'nde.

---

## 100 anti-AI-slop kuralı — linter
...
## 110 brand DESIGN.md spec'i — kategoriye göre
...
| Brand spec'leri | **110** | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 0 |
| Anti-pattern kuralları | **100** | 0 | 0 | 0 | 0 | 0 | 0 | 0 | 0 |
...
**ux-skill** · **v2.0.0-alpha.1** · Claude Code, Cursor, Windsurf ve diğer her AI coding aracının AI üretilmiş gibi okunmayan frontend çıktısı vermesi için inşa edildi.
