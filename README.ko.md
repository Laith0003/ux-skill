...
7. [120개의 반 AI-슬롭 규칙 — 린터](#120개의-반-ai-슬롭-규칙--린터)
8. [131개의 브랜드 DESIGN.md 사양 — 카테고리별](#131개의-브랜드-designmd-사양--카테고리별)
...
| **Laith0003/ux-skill (ux-skill)** | **14** | **Python 엔진 + 11개 매니페스트 + 22개 명령 + 5개 서브에이전트 + CI 린터** | **22** | **120개 regex 규칙** | **110** | **148** | **57** | **17** |
...
- **안티패턴 린터:** 120개의 결정론적 regex 규칙, CI에서 실행되며 Critical/High에서 비영(非零) 종료. 다른 어떤 곳도 결정론적 린터를 출시하지 않습니다.
...
│   ├── anti-patterns.json             120개 regex 규칙(CI 안전 린터 소스)
...
4. **출력.** 선택된 스타일, 팔레트, 타입 페어, 상위 5 모션 프리셋, 상위 12 컴포넌트, 상위 5 브랜드 본보기, 그리고 120개 안티패턴 가드레일 모두 활성을 담은 JSON 문서. 각 선택의 근거 블록까지.
5. **생성.** 다운스트림 명령(`/ux-design`, `/ux-component`, `/ux-system`, `/ux-dashboard`)이 추천을 소비하여 서브에이전트를 통해 실제 코드를 생성합니다.
6. **검증.** `/ux-lint`가 생성된 코드를 120개 regex 규칙에 대해 다시 스캔합니다. CI에서 Critical/High에 부딪히면 비영 종료.
...
- **호출(Claude Code):**
  ...
  **호출(CLI):**
  ...
  ux recommend \
    ...
    --region=mena
  ```
- **출력:** `.ux/last-recommendation.json`을 씁니다 — 선택된 스타일, 선택된 팔레트, 선택된 타입 페어, 상위 5 모션 프리셋, 상위 12 컴포넌트, 상위 5 브랜드 본보기, 120개 안티패턴 가드레일 모두 활성, 그리고 근거.
...
- **무엇:** 코드에 대해 120개 regex 규칙을 실행합니다. LLM 호출 없음. CI에서 Critical/High에 부딪히면 비영 종료. 소스: `data/anti-patterns.json`. 규칙은 A11y(23), 콘텐츠(15), 레이아웃(13), 타이포그래피(10), 색(9), 품질(9), 시각(9), 모션(8), 성능(4)을 커버합니다.
...
### `anti-patterns.json` — 120개 regex 규칙
...
전체 규칙 목록은 [120개의 반 AI-슬롭 규칙](#120개의-반-ai-슬롭-규칙--린터)에 있습니다.

### `brands/*.json` — 131개 브랜드 사양
...
전체 목록은 [131개의 브랜드 DESIGN.md 사양](#131개의-브랜드-designmd-사양--카테고리별)에 있습니다.
