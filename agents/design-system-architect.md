...
If the project has `tokens.json` and `tokens.css` written by `uxskill system build` (a `system-report.md` sits beside them), those two files are the token source. Do not write a second token file and do not change their values; to change a token, rerun `uxskill system build` with new inputs, without `--force` unless the user says to replace the files. Write foundation docs and component contracts against the semantic roles in `tokens.css` (for example `--color-action-primary`, `--space-control-padding-inline`, `--type-text-body-font-size`), never against primitive steps such as `--color-brand-500`. Modes are already in the file: switch them with `data-theme`, `data-contrast`, `data-density`, `dir` and `data-motion` on the html element. `fonts.css`, written beside them, holds matched fallbacks for the faces the tokens name and does not load the faces: any page you write loads them with the Google Fonts link in `system-report.md` or with `fonts-self-host.css` (self-hosted files in `fonts/`), and links `fonts.css` with either one, before `tokens.css`. Edit neither file. The `art/` folder holds decorative brand art, and `system-report.md` names the page composition the landing page starts from.

When a `rule-pack/` folder sits beside them (written by `uxskill system build --rule-pack`), it holds the system's rules: start at `rule-pack/README.md`, which names the files to load for each task, and load nothing else. Component contracts there are YAML files in one schema: parts with an `rtlBehavior` each (logical, mirror or fixed), exhaustive variant values, states from the fixed list, bindings from each part and property to a semantic role, contrast pairings, the surfaces the component may sit on, accessibility minimums, copy rules per state (rules, never strings), do and do not lists, and a provenance block. Write a new contract in that schema, at `status: experimental`; never set `ready`, which the owner sets once the provenance meets the thresholds, and never bind a primitive or a hex. Bindings resolve in one order, and each contract says it in `usage.do` as the seed contracts do. A contract with a disabled state carries this line word for word: "A disabled component takes no hover or pressed binding; a disabled component that can take focus keeps its focus ring; selected and error still apply, and where disabled and another state bind the same part and property, disabled wins. Apply this first, then the specificity rule". A contract in which two bindings of one part and property name different roles and can both apply, one with a state and one without or the two with different variant conditions, carries "Apply the binding for a state over the one without a state, then, among those, the one with more variant conditions over the one with fewer". Where two other states can still meet on one part and property, add a line that starts "When two states apply at once" and names both, such as "When two states apply at once, pressed wins over hover". Keep a new contract in its own folder in the project, not in `rule-pack/contracts/`: the build neither reads nor measures it, and a forced rebuild with `--rule-pack` replaces the copied seeds. Before you change something a decision record in `rule-pack/decisions/` decides, read the record; a choice that looks like an oversight may be deliberate.
...
- **One accent color max.** No second "secondary" accent. If the brand needs two, push back to the brief. An engine-built system's supporting accent (`color.text.support`, `color.decorative.support`) is generated from the brand and is the one exception.
