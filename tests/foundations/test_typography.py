...
def test_every_axis_mix_is_valid_and_passes(contrast, density, personality):
    ...
    assert report.passed and report.rules_checked == 11


def _hand():
    ts = TokenSet()
    ts.add(Token("type.f.a", "fontFamily", ["A", "serif"]))
    ts.add(Token("type.f.b", "fontFamily", ["B", "serif"]))
    ...
    ts.add(Token("type.text.body", "typography", {
        ...
        modes={"direction:rtl": {"fontFamily": "{type.f.b}", "fontSize": "{type.s.b}",
...
def test_checks_name_the_token_and_the_fix():
    ...
    assert [f.message for f in report.failures] == [
        ...
        "type.size.x is in px; sizes in rem follow the reader's default text size, so "
        "express it in rem"]
